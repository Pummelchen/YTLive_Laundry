...
```bash
tests/run.sh          # the credential-free suite: 568 checks, no camera, no credentials
...
The gates, still worth running by hand before restarting anything:
    ...
    tests/run.sh          # 568 checks, credential-free; fails if the monitor's classification,
